...
from .models import QuizQuestion, UsageByService, UsageResponse
...
class Database:
    ...
    async def fetch_fallback_questions(
        self, category: str, locale: str, limit: int
    ) -> list[QuizQuestion]:
        """Preguntas de respaldo del banco local para una categoría (orden aleatorio)."""
        if self._pool is None:
            return []
        rows = await self._pool.fetch(
            'SELECT question, options_json, correct_index, explanation '
            'FROM quiz_fallback_questions WHERE category = $1::"QuizCategory" AND locale = $2 '
