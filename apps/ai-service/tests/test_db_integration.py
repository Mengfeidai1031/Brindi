...
async def test_fetch_fallback_questions(db):
    async with db._pool.acquire() as conn:  # noqa: SLF001 - acceso directo en test
        await conn.execute("TRUNCATE TABLE quiz_fallback_questions")
        for i in range(3):
            await conn.execute(
                "INSERT INTO quiz_fallback_questions "
                "(id, category, level, locale, question, options_json, correct_index, explanation) "
                'VALUES ($1, $2::"QuizCategory", $3::"QuizLevel", $4, $5, $6::jsonb, $7, $8)',
                f"q{i}",
                "GEOGRAFIA",
                "FACIL",
                "es",
    ...
    questions = await db.fetch_fallback_questions("GEOGRAFIA", "es", 10)
    assert len(questions) == 3
    assert all(len(q.options) == 4 for q in questions)
    assert all(0 <= q.correct_index < 4 for q in questions)

    # Categoría sin preguntas -> lista vacía.
    assert await db.fetch_fallback_questions("FUTBOL", "es", 10) == []
